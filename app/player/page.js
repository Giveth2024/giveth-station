...
export default function Player() {
  ...
  const [data, setData] = useState(null);      // from localStorage
  const [media, setMedia] = useState(null);    // from backend
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        // fetch media
        const res = await fetch(`${process.env.NEXT_PUBLIC_GIVETH_SERVER_API}/api/player?id=${parsed.Id}`, {
        ...
        // Poster
        const omdbPoster = json.media.imdbID && json.media.Poster && json.media.Poster !== "N/A"
          ? `https://img.omdbapi.com/?i=${json.media.imdbID}&h=600&apikey=${process.env.NEXT_PUBLIC_OMDB_API_KEY}`
          : null;
        const directPoster = json.media.Poster && json.media.Poster !== "N/A" ? json.media.Poster : null;
  ...
  async function refreshFavoritesState(imdbId) {
    if (!user) return;
    try {
      const res = await fetch(`${process.env.NEXT_PUBLIC_GIVETH_SERVER_API}/api/favorite/${user.id}`, {
        headers: { "x-clerk-id": user.id }
      });
      const list = await res.json();
      const match = Array.isArray(list) ? list.find(f => f.imdb_id === imdbId) : null;
      if (match) {
        setFavorited(true);
        setFavDbId(match.id);
      } else {
  ...
  const stateKey = State === "movie" ? "movies" : State === "series" || State === "tv" ? "series" : null;
  const embedUrls = {
    xyz: { movies: `https://vidsrc.xyz/embed/movie/${Id}`, series: `https://vidsrc.xyz/embed/tv/${Id}` },
    cc: { movies: `https://vidsrc.cc/v3/embed/movie/${Id}?autoPlay=false&poster=true`, series: `https://vidsrc.cc/v3/embed/tv/${Id}?autoPlay=false&poster=true` }
  };
  const getEmbedUrl = () => (Id && stateKey ? embedUrls[source][stateKey] : null);
  const formatRatings = (ratings) => ratings?.map(r => `${r.Source}: ${r.Value}`).join(" | ");

  return (
    <>
      <SignedIn>
        <div className="bg-stone-950 text-stone-100 min-h-screen p-6">
          <button onClick={() => router.push("/home")} className="px-6 py-2 mb-4 bg-amber-400 text-stone-900 font-semibold rounded-lg shadow-md hover:bg-amber-500 hover:scale-105 transform transition duration-300">
            Back
          </button>
          ...
          <div className="flex flex-col md:flex-row gap-6 p-6 bg-stone-900 rounded-2xl shadow-2xl border border-amber-500">
            ...
            <div className="flex-1 flex flex-col gap-3">
              <h1 className="text-4xl md:text-5xl font-extrabold text-amber-400 drop-shadow-lg">
                {media.Title} <span className="text-stone-200 text-3xl">({media.Year})</span>
              </h1>
              {media.Genre && <p className="text-stone-300 italic text-lg">{media.Genre}</p>}
              {media.Plot && <p className="text-stone-400 text-base leading-relaxed">{media.Plot}</p>}
              ...
              <div className="mt-4 grid grid-cols-1 md:grid-cols-2 gap-2 bg-stone-800 p-4 rounded-xl border border-amber-600">
                {Object.entries(media).map(([key, value]) => {
                  if (["Poster","Title","Plot","Genre","Response"].includes(key) || !value) return null;
              ...
              <div className="flex gap-4 mt-4">
                <button onClick={toggleFavorite} disabled={savingFav} className={`px-4 py-2 rounded shadow-md transition ${favorited ? "bg-red-500 text-white" : "bg-stone-800 text-stone-200 hover:bg-stone-700"}`}>
                  {savingFav ? "Saving..." : (favorited ? "Unfavorite ❤️" : "Add to Favorites 🤍")}
                </button>
                <button onClick={() => router.push("/history")} className="px-4 py-2 rounded shadow-md bg-amber-400 text-stone-900 hover:bg-amber-500 transition">
                  View History
                </button>
              </div>
            </div>
          ...
          {getEmbedUrl() && (
            <div className="h-[100dvh] w-full mt-6 rounded-xl overflow-hidden shadow-2xl border border-amber-400">
              <iframe src={getEmbedUrl()} className="w-full h-[100dvh]" allowFullScreen title="video player" />
            </div>
          )}

          <h4 className="my-4 text-stone-300">Click another source if one doesn&apos;t work</h4>
          <div className="flex gap-4 mb-8">
            <button className={`px-4 py-2 rounded shadow-md transition ${source === "xyz" ? "bg-amber-400 text-stone-900" : "bg-stone-800 text-stone-200 hover:bg-stone-700"}`} onClick={() => setSource("xyz")}>vidsrc.xyz</button>
            <button className={`px-4 py-2 rounded shadow-md transition ${source === "cc" ? "bg-amber-400 text-stone-900" : "bg-stone-800 text-stone-200 hover:bg-stone-700"}`} onClick={() => setSource("cc")}>vidsrc.cc</button>
          </div>
        </div>
      </SignedIn>

...
// Import the specific logo for TV 2 from your public folder
import bukeddeTv2Logo from '../../../public/bukeddeTv2.png'; 

export default function BukeddeTV2Player() {
  const router = useRouter();
  const videoRef = useRef(null);
  
  // Metadata specifically for Bukedde TV 2
  const media = {
    Title: "Bukedde TV 2",
    Year: "LIVE",
    Genre: "Movies, Entertainment, Sports",
    Plot: "Bukedde TV 2 (Omukutu Gw'abantu) offers premium Luganda entertainment, featuring non-stop local movies, sports highlights, and cultural programming.",
    Language: "Luganda",
    Country: "Uganda",
    Quality: "576p"
  };

  const streamUrl = "https://stream.hydeinnovations.com/bukedde2flussonic/index.m3u8";
  const imgSrc = bukeddeTv2Logo;
  ...
  useEffect(() => {
    ...
    if (videoRef.current) {
      ...
      if (video.canPlayType('application/vnd.apple.mpegurl')) {
        ...
        // Hls.js support for Chrome/Firefox/Android
        hls = new Hls();
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="flex flex-col lg:flex-row gap-8 p-6 bg-stone-900/50 backdrop-blur-md rounded-3xl shadow-2xl border border-stone-800 hover:border-amber-500/50 transition-colors">
          ...
          <div className="flex-1 flex flex-col justify-center gap-4">
            ...
            <p className="text-stone-300 text-lg leading-relaxed max-w-2xl border-l-4 border-amber-500 pl-4 bg-stone-800/30 py-2 italic">
              {media.Plot}
            </p>
        ...
        <div className="relative group">
          ...
          <div className="relative bg-black rounded-2xl overflow-hidden shadow-2xl border border-stone-800 aspect-video lg:max-h-[75vh] mx-auto">
            <video
              ref={videoRef}
              controls
              className="w-full h-full"
              poster={imgSrc.src} // Uses the logo as a placeholder before the stream loads
              autoPlay
              muted 
            />
          </div>
